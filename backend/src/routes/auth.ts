...
export function registerAuthRoutes(app: App) {
  ...
  app.fastify.post('/api/verify-otp', {
    ...
    try {
      ...
      resend.emails.send({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Your account has been verified successfully. We're excited to have you on board!</p>
      ...
      app.logger.info({ email }, 'Welcome email sent after OTP verification');
  ...
  app.fastify.post('/api/auth/forgot-password', {
    ...
    try {
      ...
      const { resend } = await import('@specific-dev/framework');
      const resetLink = `https://p4t7z7vvqezta76k2rfz5t4kn9gytc5z.app.specular.dev/api/auth/reset-redirect?token=${encodeURIComponent(resetToken)}&email=${encodeURIComponent(email)}`;
